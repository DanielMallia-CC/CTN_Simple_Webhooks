...
def _extract_title(properties: Dict[str, Any]) -> str:
    # Business rule:
    #   Site Visit(s)_<name> -> <name>
    raw = extract_page_title(properties, fallback="Site Visit")
    cleaned = clean_event_title(
        raw,
        prefixes=[
            "Site Visit_",
            "Site Visit ",
...
def _parse_iso_to_sydney(iso_str: str) -> datetime:
    """
    Parse an ISO string from Notion and ensure it is timezone-aware
    in Australia/Sydney.
    """
...
def _extract_date_value(properties: Dict[str, Any]) -> Dict[str, Optional[str]]:
    ...
    if not prop or prop.get("type") != "date":
        raise ValueError("Site Visit missing Date & Time property")

    date_val = prop.get("date")
    if not date_val:
        raise ValueError("Site Visit Date & Time is empty")
...
def build_event_payload(
    ...
    if not start_raw:
        raise ValueError("Site Visit Date & Time missing start")
